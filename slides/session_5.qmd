...
*The consequence*: Predictions from the selected model ignore uncertainty about which model is correct.
...
*An alternative*: Model averaging acknowledges uncertainty by combining predictions across multiple models.
...
Rather than selecting model $k^*$ with, for example, minimum AIC, we form a weighted average across all $K$ candidate models.
...
*Interpretation*: The weight $w_k$ can be interpreted as the probability that model $k$ is the best model for prediction among the candidate set, given the data and assuming one of the models is correct.
...
*Weights sum to one*: By construction, $\sum_{k=1}^{K} w_k = 1\ $.

This ensures the weighted average is a convex combination of model predictions.

*Exponential decay*: Weight decreases exponentially with $\Delta_k$.
...
*Relative support*: The weight ratio $w_k / w_j = \exp[(\Delta_j - \Delta_k)/2]\ $ measures the relative support for model $k$ versus model $j$.
...
*Compute differences*: $$\Delta_1 = 0, \quad \Delta_2 = 1.6, \quad \Delta_3 = 4.8$$
...
*When multiple models have non-trivial evidential support*: 
...
*When model uncertainty is important*: If the goal is prediction with honest uncertainty quantification, model averaging accounts for both parameter uncertainty (within each model) and model uncertainty (across models).

*When model interpretation is secondary*: Model averaging produces better predictions but complicates interpretation.
...
*Procedure*: Rank models by AIC (or equivalently, by descending weight $w_k$).
...
*Interpretation*: These are the models with substantial empirical support.
...
*The candidate set matters*: Model averaging cannot compensate for a poorly chosen candidate set.
...
*Computational cost*: Fitting all $2^p$ subsets is infeasible for large $p$.
Regularization (Session 4) is more scalable for high-dimensional problems.

*Not a panacea for selection bias*: Model averaging reduces but does not eliminate selection bias.
If models are data-selected (e.g., via stepwise), averaging them does not fix the underlying problem.

*Interpretation complexity*: Averaged coefficients do not correspond to a single causal model.
